...
export function HomeHero({ activeDeckLabel, examLevel, dueCount, accountEmail, wide, onStartStudy, onStartQuiz, onSearch, onAdd, onSettings, onAccount }: HomeHeroProps) {
  ...
  return (
    <ImageBackground source={pragueHero as never} style={[styles.hero, wide && styles.heroWide]} imageStyle={styles.heroImage} resizeMode="cover">
      ...
      <HomeHeroTop activeDeckLabel={activeDeckLabel} accountEmail={accountEmail} examLevel={examLevel} onAccount={onAccount} onAdd={onAdd} onSearch={onSearch} onSettings={onSettings} />
