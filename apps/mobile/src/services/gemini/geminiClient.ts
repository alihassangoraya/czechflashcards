...
const MISSING_KEY_MESSAGE = "AI Tutor is not configured for this build. Add EXPO_PUBLIC_GEMINI_API_KEY, VITE_GEMINI_API_KEY, GEMINI_API_KEY, GOOGLE_API_KEY, or GOOGLE_GENERATIVE_AI_API_KEY to enable online tutoring; offline study still works.";
...
export async function generateGeminiText(prompt: string, fallback: string): Promise<string> {
  ...
  if (!response.ok) return `AI Tutor is unavailable right now (${response.status}). Try again later.`;
